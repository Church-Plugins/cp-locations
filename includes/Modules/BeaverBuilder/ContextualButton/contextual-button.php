...
namespace CP_Locations\Modules\BeaverBuilder\ContextualButton;
...
class Module extends \FLBuilderModule {
	...
	public function __construct() {
		parent::__construct(array(
			'name'            => __( 'CP Contextual Button', 'cp-locatons' ),
			'description'     => __( 'A location-aware button.', 'cp-locations' ),
			'category'        => __( 'Basic', 'fl-builder' ),
			'dir'             => CP_LOCATIONS_FL_MODULES_DIR . 'ContextualButton/',
			'url'             => CP_LOCATIONS_FL_MODULES_URL . 'ContextualButton/',
	...
	public function get_link() {
		$current_location = \CP_Locations\Setup\Taxonomies\Location::get_rewrite_location();

		$alternate_links = $this->settings->alternate_links;

		if ( ! empty( $alternate_links ) && $current_location ) {
			foreach ( $alternate_links as $link ) {
				if ( $link->location && in_array( $current_location['ID'], array_map( 'absint', $link->location ) ) ) {
					return $link->url;
				}
			}
		}
			...
			'general'  => array(
				...
				'fields' => array(
					...
					'click_action'   => array(
						...
						'toggle'  => array(
							'link'     => array(
								'fields' => array( 'link', 'alternate_links' ),
							),
							'lightbox' => array(
					...
					'alternate_links' => array(
						'type'  => 'form',
						'label' => __( 'Alt Location Link', 'cp-locations' ),
						'form'  => 'cp_contextual_button_link_form',
						'preview' => 'label',
						'multiple' => true
					)
				),
...
\FLBuilder::register_settings_form(
	'cp_contextual_button_link_form',
	array(
		'title' => __( 'Alt Location URL', 'cp-locations' ),
		'tabs'  => array(
			'general' => array(
				'title' => __( 'General', 'cp-locations' ),
				'sections' => array(
					'general' => array(
						'title'  => '',
						'fields' => array(
							'url' => array(
								'label' => __( 'Link', 'cp-locations' ),
								'type'  => 'link',
							),
							'location' => array(
								'label'   => __( 'Locations', 'cp-locations' ),
								'type'    => 'select',
