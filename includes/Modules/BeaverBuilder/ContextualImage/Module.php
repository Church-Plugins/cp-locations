<?php
/**
 * Contextual Image Beaver Builder Module
 *
 * @package CP_Locations
 */

namespace CP_Locations\Modules\BeaverBuilder\ContextualImage;
...
class Module extends \FLBuilderModule {
	...
	public function __construct() {
		parent::__construct(array(
			'name'            => __( 'CP Contextual Image', 'cp-locations' ),
			'description'     => __( 'Contextual image module for locations.', 'cp-locations' ),
			'category'        => __( 'Basic', 'fl-builder' ),
			'dir'             => CP_LOCATIONS_FL_MODULES_DIR . 'ContextualImage/',
			'url'             => CP_LOCATIONS_FL_MODULES_URL . 'ContextualImage/',
	...
	protected function _get_uncropped_url() {
		...
		$alternate_images = $this->settings->alternate_images;

		if ( ! empty( $alternate_images ) && $current_location ) {
			foreach ( $alternate_images as $alternate_image ) {
				if ( in_array( $current_location['ID'], array_map( 'absint', $alternate_image->location ) ) ) {
					return $alternate_image->image_src;
				}
			}
		}
 ...
 */
\FLBuilder::register_module( Module::class, array(
	'general' => array( // Tab
			...
			'general' => array(
				...
				'sections' => array(
					'general' => array(
						...
						'fields' => array(
							'image' => array(
								'label' => __( 'Image', 'cp-locations' ),
								'type'  => 'photo',
							),
							'location' => array(
								'label'   => __( 'Location', 'cp-locations' ),
								'type'    => 'select',
