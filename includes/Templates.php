 ...
 * 2) parent template, 3) plugin resources. will look in the cp-locations/
...
function get_template_hierarchy( $template, $args = [] ) {
	...
	$template_base_paths = apply_filters( 'cploc_template_paths', ( array ) cp_locations()->get_plugin_path() );
	...
	*/
	...
	if ( locate_template( [ 'cp-locations/' ] ) ) {
	...
	if ( $overrides_exist ) {
		...
		$file = locate_template( [ 'cp-locations/' . $template ], false, false );
		if ( ! $file ) {
			...
			foreach ( array_keys( $template_base_paths ) as $namespace ) {
				if ( ! empty( $namespace ) && ! is_numeric( $namespace ) ) {
					$files[] = 'cp-locations' . str_replace( $namespace, '', $template );
				}
			}
			$file = locate_template( $files, false, false );
			if ( $file ) {
				_deprecated_function( sprintf( esc_html__( 'Template overrides should be moved to the correct subdirectory: %s', 'cp-locations' ), str_replace( get_stylesheet_directory() . '/cp-locations/', '', $file ) ), '3.2', $template );
			}
		} else {
			$file = apply_filters( 'cploc_template', $file, $template );
		}
	}
	...
	return apply_filters( 'cploc_template_' . $template, $file );
}
