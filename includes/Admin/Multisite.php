...
class Multisite {
	...
	protected function actions() {
		...
		add_action( 'cmb2_admin_init', [ $this, 'network_options' ] );
	 ...
	 * @return mixed|void
	 * @since  1.0.0
	 * @update 1.1.0 - add site option for location mapping
	 *
	 * @param $id - optional location origin id to get settings for
	 *
	 * @author Tanner Moushey
	 */
	public static function get_location_site_map( $id = null ) {
		$map = wp_list_pluck( self::get_location_map_settings(), 'location_id', 'site_id' );
		$map = apply_filters( 'cploc_get_location_site_map', $map, $id );
	...
	/**
	 * Get the location map settings for content sync
	 *
	 * @since  1.1.0
	 *
	 * @param $id - optional location origin id to get settings for
	 ...
	 * @author Tanner Moushey, 9/27/23
	 */
	public static function get_location_sync_settings( $id = null ) {
		$map = wp_list_pluck( self::get_location_map_settings(), 'post_types', 'site_id' );
		$map = apply_filters( 'get_location_sync_settings', $map, $id );
	...
	/**
	 * Process settings and put into a format that is easier to work with
	 *
	 * @since  1.1.0
	 *
	 * @param $id - optional location origin id to get settings for
	 ...
	 * @author Tanner Moushey, 9/27/23
	 */
	public static function get_location_map_settings( $id = null ) {
		$options = get_site_option( 'cploc_network_options', [] );
		$map = [];
		...
		foreach( $options['location_map'] as $settings ) {
			$site_id = $location_id = false;

			// find our site id and location id
			foreach ( $settings as $key => $value ) {
				if ( false !== strpos( $key, 'site_' ) ) {
					$site_id     = str_replace( 'site_', '', $key );
					$location_id = $value;
				}
			}

			// make sure we have a site id and location id
		...
		$map = apply_filters( 'cploc_get_location_map_settings', $map, $id );
	...
	/**
	 * Add option for mapped locations and content sync
	 *
	 * @since  1.1.0
	 *
	 * @author Tanner Moushey, 9/27/23
	...
	public function network_options() {
		...
		$cmb = new_cmb2_box( [
			'id'           => $prefix . 'cploc_network_options',
			'title'        => __( 'CP Locations', 'cp-locations' ),
			'object_types' => [ 'options-page' ],
			'option_key'   => $prefix . 'network_options',
		...
		$locations = wp_list_pluck( Location::get_all_locations(), 'title', 'origin_id' );

		if ( count( $sites ) == 1 || empty( $locations ) ) {
			$cmb->add_field( [
				'type' => 'title',
				'id'   => 'cploc_no_locations',
				'name' => __( 'No Locations or Sites Found', 'cp-locations' ),
			] );

			return;
		}

		$locations[0] = __( 'No Location Mapping', 'cp-locations' );
		ksort( $locations );

		$group_field_id = $cmb->add_field( array(
			'id'          => 'location_map',
			'type'        => 'group',
			'description' => __( 'Location mapping for multisite and content sync.', 'cp-locations' ),
		...
		$post_types = [];
		foreach( cp_locations()->setup->taxonomies->location->get_object_types() as $type ) {
			$post_types[ $type ] = get_post_type_object( $type )->labels->name;
		}
		...
		foreach ( $sites as $site ) {
			...
			$cmb->add_group_field( $group_field_id, [
				...
				'id'   => 'site_' . $site->blog_id,
				'type' => 'select',
				'options' => $locations,
			] );

			$cmb->add_group_field( $group_field_id, [
				'name'    => __( 'Content to Sync', 'cp-locations' ),
				'id'      => 'post_types',
				'type'    => 'multicheck',
				'options' => $post_types,
			] );
	 ...
	 *
	 * @since  1.0.0
	 * @updated 1.1.0 to check post_types against mapping data
	 *
	...
	public static function sync_post( $post_id, $site_id ) {
		...
		if ( ! in_array( get_post_type( $post_id ), self::get_location_sync_settings( $site_id ) ) ) {
			return;
		}
