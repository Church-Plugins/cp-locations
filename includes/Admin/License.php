...
class License {
	...
	public function activate_license() {
		...
		$api_params = array(
			...
			'license'    => $license,
			'item_name'  => urlencode( CP_LIBRARY_ITEM_NAME ), // the name of our product in EDD
			'url'        => home_url()
		);

		// Call the custom API.
		$response = wp_remote_post( CP_LIBRARY_STORE_URL, array(
			'timeout'   => 15,
			'sslverify' => false,
			'body'      => $api_params
		) );
	...
	public function check_license() {
		...
		if ( $status === false && $license ) {
			...
			$api_params = array(
				...
				'item_name'  => urlencode( CP_LIBRARY_ITEM_NAME ),
				'url'        => home_url()
			);

			$response = wp_remote_post( CP_LIBRARY_STORE_URL, array( 'timeout' => 35, 'sslverify' => false, 'body' => $api_params ) );
	...
	public function plugin_updater() {
		...
		if ( ! class_exists( 'EDD_SL_Plugin_Updater' ) ) {
			include( CP_LIBRARY_PLUGIN_DIR . '/includes/updater.php' );
		}
		...
		new \EDD_SL_Plugin_Updater( CP_LIBRARY_STORE_URL, CP_LIBRARY_PLUGIN_FILE, array(
				'version'   => CP_LIBRARY_PLUGIN_VERSION,    // current version number
